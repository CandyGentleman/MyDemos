...
//  Copyright (c) 2014年 liulujie. All rights reserved.
//

#import "LJPlaceholderTextView.h"
#define kTopY 7.0
#define kLeftX 5.0
@interface LJPlaceholderTextView()<UITextViewDelegate>
...
@end
@implementation LJPlaceholderTextView
...
-(void)addMaxTextLengthWithMaxLength:(NSInteger)maxLength andEvent:(void (^)(LJPlaceholderTextView *text))limit
{
    if (maxLength>0) {
...
-(void)addTextViewBeginEvent:(void (^)(LJPlaceholderTextView *))begin{
    
    _BeginBlock=begin;
}

-(void)addTextViewEndEvent:(void (^)(LJPlaceholderTextView *))End{
    _EndBlock=End;
}
...
-(void)textViewBeginNoti:(NSNotification*)noti{
    ...
    if (_BeginBlock) {
        void(^begin)(LJPlaceholderTextView*text)=_BeginBlock;
        begin(self);
    }
}
-(void)textViewEndNoti:(NSNotification*)noti{
    ...
    if (_EndBlock) {
        void(^end)(LJPlaceholderTextView*text)=_EndBlock;
        end(self);
    }
}
...
-(void)DidChange:(NSNotification*)noti{
    ...
    if (_eventBlock && self.text.length > self.maxTextLength) {
        ...
        void (^limint)(LJPlaceholderTextView*text) =_eventBlock;
...
//        float  height=  [LJPlaceholderTextView boundingRectWithSize:CGSizeMake(_placeholdeWidth, MAXFLOAT) withLabel:_placeholder withFont:_PlaceholderLabel.font];
